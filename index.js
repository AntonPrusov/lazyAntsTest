...
function startRecursive() {
    return new Promise(ready => {
        (function recursiveSum(arr, prevSumm) {
            let arrCopy = [...arr],
                summ = prevSumm ? prevSumm : 0,
                newArr = arrCopy.splice(0, 10),
                newArrPromises = newArr.map(user => processUser(user.id));

            Promise.all(newArrPromises)
                .then(values => values.forEach(value => summ += value ))
                .then(() => arrCopy.length > 0 ? recursiveSum(arrCopy, summ) : ready(summ));
